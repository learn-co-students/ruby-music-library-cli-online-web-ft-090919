class MusicLibraryController 
  ...
  def list_songs_by_artist
    ...
    if artist = Artist.find_by_name(input)
      artist.songs.select { |song| song.artist == artist }.sort{ |a, b|  a.name <=> b.name }.uniq.each_with_index{ |song, index| puts "#{index + 1}. #{song.name} - #{song.genre.name}" }
    end
  end
  ...
  def play_song
    ...
    song = songs[index - 1]
    if index > 0  && index < songs.count && song
